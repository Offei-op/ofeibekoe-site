...
description: "A study of position embeddings in a character-level Bengio MLP, and what they actually buy you."
date: 2026-05-23
categories: [machine-learning, experiments, language-models]
image: assets/phase3-context-sweep.png
---
...
![The variant. Same architecture, with one branch added: the current position within the name gets its own embedding table and joins the concatenation. The hidden layer and output are unchanged.](assets/mlp-pe-architecture.png)
...
Full configs and JSONL logs are in the [project repo](https://github.com/your-username/position-embeddings-bengio-mlp).
...
![Test perplexity at independently-tuned best, with a width-matched control on the right (5 seeds; error bars ±1 std). MLP+PE wins regardless of how the comparison is framed.](assets/phase2-bar-chart.png)
...
![Param-matched scale sweep at context_length=4 (3 seeds; bands ±1 std). MLP+PE wins at every scale tested. The U-shape past ~5×10⁴ parameters is the data ceiling — 32K names isn't enough to keep filling larger models.](assets/phase4-scale-curve.png)
...
![Test perplexity vs context length (5 seeds; bands ±1 std). The 1.21-perplexity gap at ctx=2 collapses by ctx=6, exactly when the dot-padding pattern at the window's start becomes visible to the model.](assets/phase3-context-sweep.png)
