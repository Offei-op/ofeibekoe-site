...
![The Bengio MLP. Each character index gets turned into a learned vector by the embedding table; the vectors are stitched together and passed through the hidden layer; the output is a probability for each possible next character, and we sample from those probabilities to pick one.](assets/bengio-mlp.gif)
...
![The variant. Same architecture, with one branch added: the current position within the name gets its own embedding table and joins the concatenation. The hidden layer and output are unchanged.](assets/mlp-pe.gif)
...
![Test perplexity at independently-tuned best, with a width-matched control on the right (5 seeds; error bars ±1 std). MLP+PE wins regardless of how the comparison is framed.](assets/phase2_final_comparison.png.png)
...
![Param-matched scale sweep at context_length=4 (3 seeds; bands ±1 std). MLP+PE wins at every scale tested. The U-shape past ~5×10⁴ parameters is the data ceiling — 32K names isn't enough to keep filling larger models.](assets/phase4_width_sweep.png.png)
